...
import org.example.blogapp.models.ControlStyle
...
fun applyStyle(controlStyle: ControlStyle) {
    ...
    if (selectedIntRange != null && selectedText != null) {
        getEditor().value = getEditor().value.replaceRange(
            range = selectedIntRange, replacement = controlStyle.style
        )
...
fun applyControlStyle(
    editorControl: EditorControl, onLinkClick: () -> Unit, onImageClick: () -> Unit
) {
    when (editorControl) {
        EditorControl.Bold -> {
            applyStyle(
